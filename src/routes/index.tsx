...
function CreditsPage() {
  ...
  <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>{selected && <DialogContent className="max-h-[94vh] w-[94vw] max-w-6xl overflow-y-auto border border-border bg-background p-0 shadow-2xl sm:rounded-none [&>button]:hidden">
    ...
    <div className="grid gap-10 px-6 py-9 sm:px-10 lg:grid-cols-[minmax(260px,0.75fr)_1.25fr] lg:gap-16 lg:py-12"><div><img src={selected.image} alt={`${selected.title} cover`} width={1024} height={1024} className="aspect-square w-full object-cover" /><div className="mt-5 flex flex-wrap gap-2">{selected.roles.map((role) => <span key={role} className="border border-primary px-3 py-1.5 font-mono text-[9px] tracking-[0.18em] text-primary">{role.toUpperCase()}</span>)}</div></div>
      <div><p className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground">SINGLE — {selected.year}</p><dl className="mt-8 border-t border-border">{[["LABEL", selected.label], ["STUDIO", selected.studio], ["ROLES", selected.roles.join(" / ")]].map(([term, value]) => <div key={term} className="grid grid-cols-[90px_1fr] gap-4 border-b border-border py-4"><dt className="font-mono text-[9px] tracking-[0.2em] text-muted-foreground">{term}</dt><dd className="text-sm text-foreground/85">{value}</dd></div>)}</dl>
      <h2 className="mt-10 font-display text-xl font-semibold">Tracklist</h2><ol className="mt-4 border-t border-border">{[selected.title, "Afterimage", "Half Awake", "Everything Stays"].map((track, index) => <li key={track} className="grid grid-cols-[2rem_minmax(0,1fr)_auto_auto] items-center gap-3 border-b border-border py-3.5 text-sm"><span className="font-mono text-[10px] text-muted-foreground">{String(index + 1).padStart(2, "0")}</span><span>{track}</span><span className="font-mono text-[10px] text-muted-foreground">{["3:42", "4:08", "3:19", "5:01"][index]}</span>{index !== 2 ? <span className="grid size-5 place-items-center bg-primary font-mono text-[9px] font-semibold text-primary-foreground">M</span> : <span className="size-5" />}</li>)}</ol>
      <div className="mt-9 flex flex-wrap gap-5"><a href="https://music.apple.com" className="inline-flex items-center gap-2 border-b border-foreground pb-1 text-sm"><Music2 size={16}/> Apple Music</a><a href="https://spotify.com" className="inline-flex items-center gap-2 border-b border-foreground pb-1 text-sm"><Play size={16}/> Spotify</a><a href="https://youtube.com" className="inline-flex items-center gap-2 border-b border-destructive pb-1 text-sm text-destructive"><ExternalLink size={16}/> Video</a></div>
      <div className="mt-12"><p className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground">MORE BY {selected.artist.toUpperCase()}</p><div className="mt-4 grid grid-cols-3 gap-4">{credits.filter((item) => item.title !== selected.title).slice(0,3).map((item) => <Button key={item.title} variant="ghost" onClick={() => setSelected(item)} className="aspect-square h-auto rounded-none p-0"><img src={item.image} alt={`${item.title} cover`} className="h-full w-full object-cover" loading="lazy" /></Button>)}</div></div>
      </div></div>
